...
기간을 먼저 맞추면 이후 화면에 그대로 유지됩니다.

1. Google Ads 접속 → 우측 상단 **기간 선택 → 캠페인 시작일 ~ 어제**
2. 아래 5개 화면에서 각각 우측 상단 **다운로드 아이콘 → .csv**
...
`--tag` 로 구분하세요. `python3 scripts/browser_download.py --tag 2026H1`
